...
#    1. user_can_see_event_analytics_test.rb - we need a path where
# - the user tries to go to a event that doesn't exist,
...
# - the user goes to `sources/:id/events` and we confirm that the user can see all of the events that are generated by the `self.order_events` method in `payload_request.rb`
